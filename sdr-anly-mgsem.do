...
foreach x of varlist edu liv hrs com gov org {
  forval i = 1/5 {
    qui sum `x' if site == `i'
	local n`i' = r(N)
	local m`i' = r(mean)
...
sem (DS -> ds1 ds2 ds3 ds4) ///
    (DS <- a2 a3 fem wht mar ach edu i2 i3 liv hrs com org p2 p3 mlk gov), ///
...
forval i = 1/5 {
  esttab m1 using s`i'.csv, csv replace b(%9.2f) se(%9.2f) nogap            ///
